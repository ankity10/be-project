...
class WebPage(QWebEnginePage):
    ...
    def save_note(self, msg):
        try:
            note_dict = {"create_time": datetime.datetime.now().time().isoformat(), "note_text": msg,
                         "process_name": self.process_name, "window_title": self.window_title,
                         "note_hash": self.note_hash}
            note = Note(**note_dict)
            local_log_dict = {}
            local_log_dict = {"note_hash": self.note_hash, "note_text": msg, "process_name": self.process_name,
                              "window_title": self.window_title, "from_client_id": self.main_app.client_id}
...
class NoteWindow(QWebEngineView):
    ...
    def eventFilter(self, object, event):
        if (
                        event.type() == QEvent.ChildAdded and object is self and event.child().isWidgetType() and self.child == None):
...
class LoginWindow(QWidget):
    ...
    def signup_method(self):
        ...
        response = requests.post(self.main_app.signup_url,
                                 json={'username': self.new_username, 'password': self.new_password,
                                       'client_id': client_id})
        data = response.json()
        print(data)
        if ('success' in data):
