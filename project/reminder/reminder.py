...
class Reminder(QWidget):
    ...
    def store_reminder(self):
        ...
        reminder_dict_info = {"note_hash": note_hash, "window_title": window_title,
                              "process_name": process_name, "event_name": self.event_title,
                              "repetition": self.repetition_text,
    ...
    def repetition_selection_method(self):
        if (self.repetition_text == 1):
            self.target_date_selected.setDate(self.target_date_selected.year(), self.target_date_selected.month(),
                                              self.target_date_selected.day() + 1)
        elif (self.reminder_text == 2):
            self.target_date_selected.setDate(self.target_date_selected.year(), self.target_date_selected.month(),
                                              self.target_date_selected.day() + 7)
        elif (self.reminder_text == 3):
            self.target_date_selected.setDate(self.target_date_selected.year(), self.target_date_selected.month() + 1,
                                              self.target_date_selected.day())

    def reminder_selection_method(self):
        if (self.reminder_text == 1):
            self.target_time_selected.setHMS(self.target_time_selected.hour(), self.target_time_selected.minute() - 5,
                                             0)
        elif (self.reminder_text == 2):
            self.target_time_selected.setHMS(self.target_time_selected.hour(), self.target_time_selected.minute() - 10,
                                             0)
        elif (self.reminder_text == 3):
            self.target_time_selected.setHMS(self.target_time_selected.hour() - 1, self.target_time_selected.minute(),
                                             0)
        elif (self.reminder_text == 4):
            self.target_time_selected.setHMS(self.target_time_selected.hour() - 2,
                                             self.target_time_selected.minute() - 5, 0)
